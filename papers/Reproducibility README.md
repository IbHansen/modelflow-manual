...
All exhibits generated by the Jupyter Notebooks that form the book and these books have all of the code for all ofthe outputs.  Although Figures and tables are not numbered per se, the code  that generates them appears just before the output itself. Although the notebooks are meant to be executed consecutively to produce the book, each also can be executed as a standalone, although some expect an output file generated by the execution of an earlier Notebook. 
